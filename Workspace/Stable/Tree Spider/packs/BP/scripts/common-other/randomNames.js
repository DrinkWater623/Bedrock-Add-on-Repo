...
export function makeRandomName (custom = {}) {
    ...
    const looksPronounceable = (s) => {
        ...
        if (/yy|ww|xx|hh|jj|qq|vv|yh|yj|yc|yg|yq|yv|yx|yw|iw|ih|ii|aa|uu/.test(name)) return false;   // avoid awkward doubles
        ...
        if (/shsh|chch|thth|phph|ququ/.test(name)) return false;   // avoid doubled digraphs
        return true;
    };
