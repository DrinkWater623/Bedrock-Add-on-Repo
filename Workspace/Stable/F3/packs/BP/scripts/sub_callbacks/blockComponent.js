...
function verifyLastInteractInfoRelated (lastInteractInfo, itemTypeId, newBlock, alert) {
    //dev.alertLog(`§dVerifying lastInteractInfo`, alert);

    if (lastInteractInfo.tick === 0) { dev.alertError(`Missing lastInteractInfo`, alert); return false; }

    //1st check for staleness
    const currentTick = system.currentTick;
    if (currentTick - lastInteractInfo.tick > 5) {
        dev.alertError(`lastInteractInfo is stale. currentTick:${currentTick} - lastTick:${lastInteractInfo.tick}>5`, alert);
        return false;
    }

    //2nd check for missing info
    if (!lastInteractInfo.faceLocation) { dev.alertError('Missing faceLocation'); return false; }
    if (!lastInteractInfo.blockLocation) { dev.alertError('Missing blockLocation'); return false; }
    if (!newBlock.dimension.isChunkLoaded(lastInteractInfo.blockLocation)) { dev.alertError('Chunk is not loaded'); return false; }
    if (!newBlock.dimension.getBlock(lastInteractInfo.blockLocation)) { dev.alertError('Cannot get block'); return false; }

    //3nd check for item type match    
    if (itemTypeId !== lastInteractInfo.itemTypeId) {
        dev.alertError(`itemStackBlock (${itemTypeId}) !== lastItemStack (${lastInteractInfo.itemTypeId})`, alert);
        return false;
    }

    //3rd check for adjacency
    if (!Vector3Lib.isAdjacent(newBlock.location, lastInteractInfo.blockLocation)) {
        dev.alertError(`location (${Vector3Lib.toString(newBlock.location, 1, true)}) is not adjacent to lastBlockLocation (${Vector3Lib.toString(lastInteractInfo.blockLocation, 1, true)})`, alert);
        return false;
    }
...
export function bar_onPlace (event) {
    ...
    if (!verifyLastInteractInfoRelated(lastInteractInfo, itemTypeId, newBlock, alert)) { dev.alertError('verifyLastInteractInfoRelated failed', alert); return; }
