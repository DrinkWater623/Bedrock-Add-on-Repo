...
export class FaceLocationGrid {
     ...
     */
    constructor(block, faceLocation, faceOverride = '') {
        ...
        if (faceOverride) {
            faceOverride = faceOverride.toLowerCase();

            if (![ 'down', 'up', 'south', 'north', 'east', 'west' ].includes(faceOverride))
                throw new Error(`Invalid Face Override (${faceOverride})`);

            this.blockFace = faceOverride;
        }
        else {
            //TODO: if block is a minecraft slab... this may not work correctly
            //      which is why I added the faceOverride for my custom slabs
            //      but for official size slabs, see how you can tell
            //      most likely if they have the faceLocation then they have the face touched
            //      this this may be a mute point.
        ...
        if ([ 'up', 'down' ].includes(this.blockFace)) {
            this.xDelta = (delta3.x * -1);
            this.yDelta = (delta3.z * -1);
        }
        else {
            ...
            if ([ 'south', 'west' ].includes(this.blockFace)) {
                this.xDelta *= -1;
            }

            this.yDelta = delta3.y;
        }
