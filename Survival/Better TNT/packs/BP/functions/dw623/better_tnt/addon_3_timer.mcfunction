...
# Every 5 ticks
    ...
    execute if score mod bt_timer matches 0 as @e[family=dw623_tnt,family=in_block,family=!activated] run function dw623/better_tnt/query_block_events
##############################################################
# Every 10 ticks
    scoreboard players operation mod bt_timer = timer bt_timer
    scoreboard players operation mod bt_timer %= n10 bt_timer
    execute if score mod bt_timer matches 0 run function dw623/better_tnt/addon_4_status_set
    ...
    execute if score mod bt_timer matches 0 as @e[family=dw623_tnt,family=!vanilla,scores={bt_status=1..}] run function dw623/better_tnt/check_status
##############################################################
# Every 20 ticks or reset
    scoreboard players operation mod bt_timer = timer bt_timer
    scoreboard players operation mod bt_timer %= n20 bt_timer
    execute if score mod bt_timer matches 0 run function dw623/better_tnt/addon_5_guard_set
    # Get Active Count
