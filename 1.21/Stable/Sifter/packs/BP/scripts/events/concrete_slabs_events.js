 ...
 * @returns 
 */
export function concrete_slabs_playerPlaceOrIntersectBlock_Handler (event) {
    const debug = true;
    ...
    //if (event.touchBlockStates.has)
    const traitsBlockFace = !event.touchBlockStates[ stateName ] ? event.touchFace : event.touchBlockStates[ stateName ].toString();
    ...
    */
    ...
    //can only be... 
    chatLog.log('  ==> Regular-ish Block', debug);
    return placeAdjacentOtherBlock(event, debug);
}
//==============================================================================
/**
...
function placeAdjacentOtherBlock (event, debug = false) {
    ...
    if (
        ...
        if (debug) world.sendMessage(`  §b==> Opposite or Same Face`);

        //if opposite, check grid for other types of placement
        if (blockFaceTrait != event.touchFace) {

            const grid4 = event.grid_4x4;
            if (debug) world.sendMessage(`  §b==> ${Vector2Lib.toString(grid4, 0, true)}`);
